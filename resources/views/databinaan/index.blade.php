       ...
       <div class="col-12">
          <div class="card">
             <div class="card-header d-flex justify-content-between flex-wrap">
                ...
                <div class="">
                    <a href="{{route('databinaan.add')}}" class="text-center btn btn-primary btn-icon mt-lg-0 mt-md-0 mt-3">
                        <i class="btn-inner">
                      ...
                      <tbody>
                        @php $no=1; @endphp
                         ...
                         <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $binaan->nama }}</td>
                            <td>{{ $binaan->jenis_kelamin }}</td>
                            <td>{{ $binaan->namakasus }}</td>
                            <td>{{ $binaan->nama_wisma }}</td>
                            <td>
                               <a href="{{ route('databinaan.show', $binaan->idbinaan) }}" class="btn btn-xs btn-icon btn-info" data-toggle="tooltip" data-placement="top" title="Detail Data">
                                ...
                                <a href="{{ route('databinaan.edit', $binaan->idbinaan) }}" class="btn btn-xs btn-icon btn-warning" data-toggle="tooltip" data-placement="top" title="Edit Data">
                                    <span class="btn-inner">
                                        <svg class="icon-32" width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="javascript:void(0);" class="btn btn-xs btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete Data" onclick="confirmDelete('{{ $binaan->idbinaan }}')">
                                    <span class="btn-inner">
                                        <svg class="icon-32" width="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function confirmDelete(userId) {
    Swal.fire({
        ...
        if (result.value) {
            axios.delete("{{ route('databinaan.destroy', '') }}/" + userId)
                .then(response => {
                    Swal.fire(
                        'Berhasil!',
                        'Data Binaan Yayasan telah dihapus.',
                        'success'
                    ).then(() => {
                        location.reload();
                    });
